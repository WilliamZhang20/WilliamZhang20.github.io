...
I represented each hit with a compact strike command

$$
m=[d_x,d_y,\alpha,\tau/\tau_{\max}],
$$

containing direction, strength, and duration. A higher-level model takes
the pre-impact JEPA latent and those strike features, then predicts both the
...
The $K$ heads form a small ensemble. Planning tries a grid of goal-relative
strike commands and minimizes endpoint error plus ensemble disagreement:
...
older D4RL and Fetch environments used by the literature anchors. The AntMaze
UMaze controller additionally uses a map-distilled discrete router, so that
result is supervision-dependent rather than a pure learned-topology result.
...
<div class="caption">
  Adroit Door rollout using the phase-conditioned inverse controller evaluated in the benchmark.
</div>
