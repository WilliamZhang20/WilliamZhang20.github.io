...
{% include figure.liquid path="assets/img/jepa/fetchslide_event_pipeline.svg" class="img-fluid rounded z-depth-1" alt="FetchSlide event-model pipeline from pre-impact state through goal-frame feature construction, endpoint scoring, and selected-strike execution" zoomable=true %}

<div class="caption">
  FetchSlide event model. Candidate strikes and live puck/goal geometry are
  converted to the goal frame; the ensemble predicts each stopping endpoint and
  its uncertainty, then the controller executes the lowest-cost strike.
</div>
