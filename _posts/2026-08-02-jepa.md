...
such as pushing, drone control, and maze navigation, but reliable planning through contact-rich,
...
The pipeline is reward-free and future-conditioned; it is not demonstration-free, and random exploration did
...
At runtime the flow produces several complete action chunks. For the FetchPush
and FetchPickAndPlace controllers that retain online JEPA ranking, runtime
selection is:

The frozen JEPA rolls them forward, and the controller selects
...
On continuous dynamics, the learned predictor can directly evaluate action sequences. For precise local contact, future-conditioned action priors provide plausible chunks for the predictor to rank. Ballistic motion benefits from predicting an event endpoint rather than every intermediate frame, while long-horizon navigation and manipulation require explicit temporal, physical-mode, or task-level structure.
...
The previous experiments demonstrated that JEPA has potential on hard tasks, especially when trained as a specialist.
...
Another next step is to train more generalist policies while extending the
evaluation to harder tasks, including block and [egg manipulation](https://robotics.farama.org/envs/shadow_dexterous_hand/manipulate_egg/)
in a stationary hand.